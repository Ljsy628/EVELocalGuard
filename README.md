...
声音告警默认开启。可以在 `报警选项` 里取消勾选 `声音` 来关闭声音，只保留弹窗/日志。`测试声音` 按钮可以直接播放一次特殊告警音效。触发真实告警后，声音会连续播放，直到点击弹窗里的 `确认并停止声音`，或主界面的 `确认告警`。
...
截图识别无法承诺 100% 零错误。想尽量接近稳定告警：

1. 只框本地成员列表，不框标题、人数、聊天内容和大面积背景。
2. 保持 `标记列宽 = 0`，程序会自动扫描左侧和右侧图标区。
3. 把 `确认帧` 设为 `3`，减少瞬间画面变化导致的误报。
4. 如果标题或背景仍被识别成白名，把 `行内容像素` 提高到 `35` 或 `45`。
5. 如果某些暗色蓝/绿标仍被当白名，把 `标记像素` 降到 `4` 或 `5`。
