...
screen and alerts when EVE Online's local member list contains hostile rows.
Blue, purple, and green rows are treated as friendly. Red and orange rows are
treated as threats. Rows without a friendly marker are ignored by default.
"""
...
def load_config() -> Dict:
    ...
    try:
        ...
        if isinstance(scan, dict) and int(scan.get("detector_version", 1)) < DEFAULT_CONFIG["scan"]["detector_version"]:
            ...
            scan["marker_x"] = DEFAULT_CONFIG["scan"]["marker_x"]
            scan["marker_width"] = DEFAULT_CONFIG["scan"]["marker_width"]
...
def normalize_ntfy_server(value: str) -> str:
    server = str(value or "").strip().rstrip("/")
    if not server:
        return DEFAULT_CONFIG["scan"]["ntfy_server"]
...
def send_ntfy_message(server: str, topic: str, title: str, message: str, priority: int = 5) -> None:
    topic = normalize_ntfy_topic(topic)
    if not topic:
        raise ValueError("ntfy topic 为空")

    url = normalize_ntfy_server(server) + "/" + urllib.parse.quote(topic, safe="")
...
class LocalGuardApp:
    ...
    def _build_ui(self) -> None:
        ...
        subtitle = ttk.Label(
            outer,
            text="蓝/紫/绿视为友方；红/橙视为威胁。白/中立默认不报警。",
            foreground="#555555",
        )
        ...
        mobile_frame = ttk.LabelFrame(outer, text="手机推送 / MacroDroid")
        mobile_frame.pack(fill="x", pady=(0, 10))
        ttk.Checkbutton(mobile_frame, text="启用 ntfy 手机告警", variable=self.vars["mobile_push"]).grid(
            row=0,
            column=0,
            sticky="w",
            padx=8,
        ...
        ttk.Label(mobile_frame, text="服务器").grid(row=0, column=1, sticky="e", padx=(8, 4), pady=6)
        ttk.Entry(mobile_frame, textvariable=self.vars["ntfy_server"], width=24).grid(
            row=0,
            column=2,
            sticky="ew",
            padx=(0, 8),
        ...
        ttk.Button(mobile_frame, text="测试手机推送", command=self.test_mobile_push).grid(
            row=0,
            column=5,
            sticky="e",
            padx=8,
        ...
        mobile_frame.columnconfigure(4, weight=1)
    ...
    def options_from_vars(self) -> DetectorOptions:
        ...
        return DetectorOptions(
            marker_x=parse_int(self.vars["marker_x"].get(), defaults["marker_x"], 0, 2000),
            marker_width=parse_int(self.vars["marker_width"].get(), defaults["marker_width"], 0, 4000),
            min_pixels_per_row=parse_int(self.vars["min_pixels_per_row"].get(), defaults["min_pixels_per_row"], 1, 500),
            min_row_content_pixels=parse_int(
                self.vars["min_row_content_pixels"].get(),
                defaults["min_row_content_pixels"],
                1,
                2000,
            ),
            row_merge_gap=parse_int(self.vars["row_merge_gap"].get(), defaults["row_merge_gap"], 0, 30),
            min_cluster_height=parse_int(self.vars["min_cluster_height"].get(), defaults["min_cluster_height"], 1, 100),
            max_cluster_height=parse_int(self.vars["max_cluster_height"].get(), defaults["max_cluster_height"], 1, 300),
    ...
    def handle_result(self, result: ThreatResult) -> None:
        ...
        defaults = DEFAULT_CONFIG["scan"]
        required_frames = parse_int(self.vars["required_frames"].get(), defaults["required_frames"], 1, 20)
        clear_frames = parse_int(self.vars["clear_frames"].get(), defaults["clear_frames"], 1, 30)
        cooldown = parse_int(self.vars["cooldown_seconds"].get(), defaults["cooldown_seconds"], 0, 3600)
    ...
    def mobile_push_enabled(self) -> bool:
        return bool(self.vars["mobile_push"].get())

    def send_mobile_message(self, title: str, message: str, priority: int = 5) -> bool:
        if not self.mobile_push_enabled():
            return False
        try:
            send_ntfy_message(
            ...
            self.log(f"手机推送失败：{exc}")
            return False

    def send_mobile_alert_start(self, result: ThreatResult, summary: str) -> None:
        if self.mobile_alert_sent:
            return
        message = (
            "EVE_ALERT_START\n"
            f"threats={result.count}\n"
            f"explicit={result.explicit_count}\n"
            f"neutral={result.neutral_count}\n"
            f"{summary}"
        )
        if self.send_mobile_message("EVE_ALERT_START", message, priority=5):
            self.mobile_alert_sent = True
            self.log("已发送手机告警：EVE_ALERT_START")

    def send_mobile_alert_stop(self) -> None:
        if not self.mobile_alert_sent:
            return
        if self.send_mobile_message("EVE_ALERT_STOP", "EVE_ALERT_STOP", priority=3):
            self.log("已发送手机解除：EVE_ALERT_STOP")
        self.mobile_alert_sent = False

    def test_mobile_push(self) -> None:
        self.save_config_from_vars()
        ...
        if self.send_mobile_message("EVE_ALERT_START", message, priority=5):
            self.log("测试手机推送已发送。")
            messagebox.showinfo(APP_NAME, "测试手机推送已发送。")
        else:
            messagebox.showerror(APP_NAME, "测试手机推送失败，请查看日志。")
